...
'''
# 更新nodes中的cluster为最新的二部图聚类结果
nodes = np.loadtxt(path4 + 'rls14_nodes0729.csv', skiprows=1, delimiter=",", dtype=str)
bi_cluster_res = np.loadtxt(path4 + 'final.csv', delimiter=",", dtype=str)
dict_biRes = {}
for i in xrange(len(bi_cluster_res)):
    dict_biRes[bi_cluster_res[i][0]] = bi_cluster_res[i][1]

for i in xrange(len(nodes)):
    cID = dict_biRes[nodes[i][0]]
    nodes[i][2] = cID

np.savetxt(path4 + 'rls14_nodes0729.csv', nodes, delimiter=',', fmt='%s')
'''
...
# 获取每个cluster的中心节点，及其属性(com or repo)
def obtain_center(coms_cmt, repos_cmt, cid):
    print 'coms_cmt', coms_cmt
    com_max = max(coms_cmt.items(), key=lambda x: x[1])
    repo_max = max(repos_cmt.items(), key=lambda x: x[1])
    if com_max[1] >= repo_max[1]:
        return [cid, com_max[0], com_max[1], 'com']
    else:
        return [cid, repo_max[0], repo_max[1], 'repo']


# 获取每个cluster的按照贡献排名前8的公司、repo列表
def obtain_top_8(coms_cmt, repos_cmt, cid):
    top_8 = []
    coms_cmt = sorted(coms_cmt.items(), key=lambda x: x[1], reverse=True)
    repos_cmt = sorted(repos_cmt.items(), key=lambda x: x[1], reverse=True)
    print 'coms_cmt', coms_cmt
    ...
    for i in xrange(0, len(coms_cmt)):
        if i < 8:
            top_8.append([cid, coms_cmt[i][0], 'com'])
        else:
            break

    for j in xrange(0, len(repos_cmt)):
        if j < 8:
            top_8.append([cid, repos_cmt[i][0], 'repo'])
        else:
            break
    return top_8
...
pre = obtain_representative(coms_cmt, repos_cmt, sum_cmt, CID)
representatives.extend(pre)
t8 = obtain_top_8(coms_cmt, repos_cmt, sum_cmt, CID)
top_8.extend(t8)
...
np.savetxt(path4 + 'cluster_centers0729.csv', centers, delimiter=',', fmt='%s')
np.savetxt(path4 + 'cluster_representatives0729.csv', representatives, delimiter=',', fmt='%s')
np.savetxt(path4 + 'cluster_top0729.csv', top_8, delimiter=',', fmt='%s')
